...
contract Moonwell4626EthLiveSystemBaseTest is Configs {
    ...
    function testSetup() public view {
        assertEq(address(router.weth()), addresses.getAddress("WETH"), "incorrect WETH address");
        assertEq(address(ethVault.asset()), addresses.getAddress("WETH"), "incorrect underlying asset");
        assertEq(address(ethVault.mToken()), addresses.getAddress("MOONWELL_WETH"), "incorrect mToken");
        assertEq(address(ethVault.rewardRecipient()), address(this), "incorrect reward recipient");
        assertEq(address(ethVault.comptroller()), addresses.getAddress("UNITROLLER"), "incorrect moontroller");
        assertEq(ethVault.totalSupply(), 0, "incorrect totalSupply");

        assertEq(address(vault.asset()), addresses.getAddress("cbETH"), "incorrect underlying asset");
        assertEq(address(vault.mToken()), addresses.getAddress("MOONWELL_cbETH"), "incorrect mToken");
        assertEq(address(vault.rewardRecipient()), address(this), "incorrect reward recipient");
        assertEq(address(vault.comptroller()), addresses.getAddress("UNITROLLER"), "incorrect moontroller");
    ...
    function testMintmWeth4626SharesUsingRouter(uint256 amount) public {
        amount = _bound(amount, 1_000e18, _getMaxSupplyAmount(addresses.getAddress("MOONWELL_WETH")) - 100e18);
    ...
    function testMintExcessWethRefunded() public {
        ...
        assertEq(ethVault.balanceOf(address(this)), shares, "incorrect eth 4626 share balance");
        assertEq(ethVault.totalSupply() - startingTotalSupply, shares, "incorrect eth 4626 total supply");
        assertEq(weth.balanceOf(address(this)), amount, "not enough refunded");
        assertEq(
            weth.allowance(address(router), addresses.getAddress("MOONWELL_WETH")),
            0,
            "allowance not zero after minting through router"
        );
        assertEq(address(router).balance, 0, "router balance not zero after minting through router");
    }
    ...
    function testWithdrawEthFromEth4626Vault(uint256 amount) public {
        ...
        assertEq(address(this).balance - startingBalance, amount, "incorrect amount out");
        assertEq(ethVault.totalSupply(), startingTotalSupply - shares, "incorrect total supply");
    }
    ...
    function testRedeemEthFromEth4626Vault(uint256 amount) public {
        ...
        ethVault.redeem(ethVault.balanceOf(address(this)), address(this), address(this));
    ...
    function testMintcbEth4626Shares(uint256 amount) public {
        uint256 maxSupplyAmount = _getMaxSupplyAmount(addresses.getAddress("MOONWELL_cbETH"));
        maxSupplyAmount = maxSupplyAmount > 1.0000001e18 ? maxSupplyAmount - 1e18 : maxSupplyAmount;
        ...
        assertEq(vault.balanceOf(address(this)), shares, "incorrect 4626 share balance");
    ...
    function testMintRouterMaxAmountInLtMaxFails(uint256 amount) public {
        amount = _bound(0, 10 * 1e18, _getMaxSupplyAmount(addresses.getAddress("MOONWELL_WETH")) - 100e18);
    ...
    function testDepositSharesAmountOutLtMinFails(uint256 amount) public {
        ...
        router.deposit{value: amount}(ethVault, address(this), amount, shares + 1);
    }
    ...
    function testDepositSharesAmountOutSucceeds(uint256 amount) public {
        ...
        /// eth balance
        assertEq(address(this).balance, 0, "incorrect balance after depositing shares");
        assertEq(address(router).balance, 0, "router balance not zero after minting through router");
