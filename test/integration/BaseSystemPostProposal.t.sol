...
contract BaseSystemPostProposalTest is PostProposalCheck, Configs {
    ...
    function testOraclesReturnCorrectValues() public view {
        ...
        unchecked {
            for (uint256 i = 0; i < cTokenConfigs.length; i++) {
                assertGt(
                    oracle.getUnderlyingPrice(MToken(addresses.getAddress(cTokenConfigs[i].addressesString))),
                    1,
    ...
    function testUpdateEmissionConfigSupplyUsdcSuccess() public {
        ...
        assertEq(config.supplyEmissionsPerSec, 1e18, "supply emissions per second incorrect");
    }
    ...
    function testUpdateEmissionConfigBorrowUsdcSuccess() public {
        ...
        assertEq(config.borrowEmissionsPerSec, 1e18, "well per second incorrect");
    }
    ...
    function testMintMTokenSucceeds() public {
        ...
        assertEq(err, 0, "Error getting account liquidity");
        assertApproxEqRel(liquidity, 80e18, 1e15, "liquidity not within .1% of $80");
        assertEq(shortfall, 0, "Incorrect shortfall");
    ...
    function testMintcbETHmTokenSucceeds() public {
        ...
        MErc20Delegator mToken = MErc20Delegator(payable(addresses.getAddress("MOONWELL_cbETH")));
        uint256 startingTokenBalance = token.balanceOf(address(mToken));
        ...
        comptroller.enterMarkets(mTokens);
        assertTrue(comptroller.checkMembership(sender, MToken(addresses.getAddress("MOONWELL_cbETH"))));
        /// ensure sender and mToken is in market
    ...
    function testBorrowOtherMTokenSucceeds() public {
        ...
        assertEq(sender.balance, borrowAmount, "incorrect ether amount borrowed");
    ...
    function testSupplyUsdcReceivesRewards(uint256 toWarp) public {
        ...
        MultiRewardDistributorCommon.RewardInfo[] memory rewards =
            mrd.getOutstandingRewardsForUser(MToken(addresses.getAddress("MOONWELL_USDBC")), address(this));

        assertEq(rewards[0].emissionToken, well);
        assertLe(rewards[0].totalAmount, toWarp * 1e18, "Total rewards not LT warp time * 1e18");
        assertLe(rewards[0].supplySide, toWarp * 1e18, "Supply side rewards not LT warp time * 1e18");
    ...
    function testBorrowUsdcReceivesRewards(uint256 toWarp) public {
        ...
        assertLe(rewards[0].totalAmount, toWarp * 1e18 + toWarp, "Total rewards not less than warp time * 1e18");
        assertLe(rewards[0].borrowSide, toWarp * 1e18, "Borrow side rewards not less than warp time * 1e18");
    }
    ...
    function testSupplyBorrowUsdcReceivesRewards(uint256 toWarp) public {
        ...
        assertLe(rewards[0].borrowSide, toWarp * 1e18, "Borrow side rewards not less than warp time * reward speed");
        assertLe(rewards[0].supplySide, toWarp * 1e18, "Supply side rewards not less than warp time * reward speed");
    }
    ...
    function testLiquidateAccountReceivesRewards(uint256 toWarp) public {
        ...
        _liquidateAccount(liquidator, address(this), MErc20(address(mToken)), 1e6);
        ...
        assertGt(toWarp * 1e18 * 2, rewards[0].totalAmount, "Total rewards not less than or equal to upper bound");
        assertLe(rewards[0].borrowSide, toWarp * 1e18, "Borrow side rewards not less than upper bound");
        assertLe(rewards[0].supplySide, (toWarp * 1e18) / 2, "Supply side rewards not less than upper bound");
    }
    ...
    function testAddCloseToMaxLiquidity() public {
        ...
        uint256 usdcMintAmount = _getMaxSupplyAmount(addresses.getAddress("MOONWELL_USDBC"));
        uint256 wethMintAmount = _getMaxSupplyAmount(addresses.getAddress("MOONWELL_WETH")) - 100e18;
        uint256 cbEthMintAmount = _getMaxSupplyAmount(addresses.getAddress("MOONWELL_cbETH")) - 100e18;
        ...
        uint256 expectedMinLiquidity =
            (((usdcMintAmount * 1e12) * 8) / 10) + wethMintAmount * 1_000 + cbEthMintAmount * 1_200;
    ...
    function testMaxBorrowWeth() public returns (uint256) {
        ...
        uint256 borrowAmount = _getMaxSupplyAmount(addresses.getAddress("MOONWELL_WETH"));
    ...
    function testMaxBorrowcbEth() public {
        ...
        uint256 borrowAmount = _getMaxSupplyAmount(addresses.getAddress("MOONWELL_cbETH"));
        address mcbeth = addresses.getAddress("MOONWELL_cbETH");

        assertEq(MErc20(mcbeth).borrow(borrowAmount), 0);
        assertEq(IERC20(addresses.getAddress("cbETH")).balanceOf(address(this)), borrowAmount);
    }

    function testMaxBorrowUsdc() public {
        testAddCloseToMaxLiquidity();

        uint256 borrowAmount = _getMaxSupplyAmount(addresses.getAddress("MOONWELL_USDBC"));
        address mUSDbC = addresses.getAddress("MOONWELL_USDBC");

        assertEq(MErc20(mUSDbC).borrow(borrowAmount), 0);
        assertEq(IERC20(addresses.getAddress("USDBC")).balanceOf(address(this)), borrowAmount);
    }
    ...
    function testRepayBorrowBehalfWethRouter() public {
        ...
        router = new WETHRouter(WETH9(addresses.getAddress("WETH")), MErc20(addresses.getAddress("MOONWELL_WETH")));
