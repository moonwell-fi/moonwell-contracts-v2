...
contract MoonwellViewsV1Test is Test, PostProposalCheck {
    ...
    function setUp() public override {
        ...
        TransparentUpgradeableProxy proxy =
            new TransparentUpgradeableProxy(address(viewsContract), proxyAdmin, initdata);
    ...
    function testMarketsSize() public view {
        MoonwellViewsV1.Market memory _market =
            viewsContract.getMarketInfo(MToken(0x091608f4e4a15335145be0A279483C0f8E4c7955));

        assertEq(_market.isListed, true);
    }

    function testUserVotingPower() public view {
        MoonwellViewsV1.UserVotes memory _votes = viewsContract.getUserVotingPower(user);

        assertEq(
            _votes.stakingVotes.votingPower + _votes.tokenVotes.votingPower + _votes.claimsVotes.votingPower,
            5000001 * 1e18
        );
    }

    function testUserStakingInfo() public view {
        MoonwellViewsV1.UserStakingInfo memory _stakingInfo = viewsContract.getUserStakingInfo(user);
    ...
    function testUserRewards() public {
        MoonwellViewsV1.Rewards[] memory _rewards = viewsContract.getUserRewards(user);

        for (uint256 index = 0; index < _rewards.length; index++) {
            bool exists = userTotalRewards[_rewards[index].rewardToken] > 0;
            userTotalRewards[_rewards[index].rewardToken] = userTotalRewards[_rewards[index].rewardToken]
                + (_rewards[index].supplyRewardsAmount + _rewards[index].borrowRewardsAmount);

            if (!exists) {
                userRewardTokens.push(_rewards[index].rewardToken);
            }
        }
        assertEq(userTotalRewards[0x511aB53F793683763E5a8829738301368a2411E3], 11526274217013010874);
        assertEq(userTotalRewards[0x0000000000000000000000000000000000000000], 575610171267701893);
    }
}
