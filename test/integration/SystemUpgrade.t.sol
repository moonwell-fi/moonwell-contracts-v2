...
contract SystemUpgradeUnitTest is PostProposalCheck {
    ...
    function testSystemUpgradeAsTemporalGovernorSucceeds() public {
        ...
        assertEq(
            bytes32(
                uint256(
                    uint160(newProxyImplementation)
                )
            ),
            data
        );
    ...
    function testSystemProxyAdminChangeAsTemporalGovernorSucceeds() public {
        ...
        assertEq(
            newProxyAdmin.getProxyAdmin(proxy),
            address(newProxyAdmin)
        );

        /// ensure that the proxy admin is set correctly
        assertEq(
            bytes32(uint256(uint160(address(newProxyAdmin)))),
            data
        );
    }
