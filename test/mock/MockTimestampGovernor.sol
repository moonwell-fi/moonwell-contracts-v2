...
contract MockTimestampGovernor {
    /// @notice The name of this contract
    string public constant name = "Mock Timestamp Governor";
    ...
    uint public quorumVotes = 100_000_000 * 1e18; // 100,000,000 WELL

    /// @notice The number of votes required in order for a voter to become a proposer
    uint public proposalThreshold = 400_000 * 1e18; // 400,000 WELL
    ...
    struct Proposal {
        ...
        bytes[] calldatas;
        /// @notice The timestamp at which voting began: holders must have delegated their votes prior to this timestamp
        uint startTimestamp;
    ...
    event StartTimestampSet(uint proposalId, uint startTimestamp);
    ...
    function propose(
        ...
        require(
            _getVotingPower(msg.sender, sub256(block.timestamp, 1)) >
                proposalThreshold,
    ...
    function cancel(uint proposalId) public {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(
            _getVotingPower(proposal.proposer, sub256(block.timestamp, 1)) <
                proposalThreshold,
    ...
    function _castVote(
        ...
        if (proposal.startTimestamp == 0) {
            proposal.startTimestamp = block.timestamp - 1;
            emit StartTimestampSet(proposalId, block.timestamp);
        }
        ...
        uint votes = _getVotingPower(voter, proposal.startTimestamp);
    ...
    function _getVotingPower(
        ...
        /// Get votes from the xWELL contract
        uint96 wellVotes = well.getPastVotes(voter, blockNumber);

        return uint(wellVotes);
    }
