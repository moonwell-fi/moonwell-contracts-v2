...
contract MockDistributionManager is IDistributionManager {
    ...
    function __DistributionManager_init_unchained(address emissionManager, uint256 distributionDuration) internal {
        ...
        DISTRIBUTION_END = block.number.add(distributionDuration);
    ...
    function _updateAssetStateInternal(
        ...
        assetConfig.lastUpdateTimestamp = uint128(block.number);
    ...
    function _getAssetIndex(
        ...
        if (
            emissionPerSecond == 0 || totalBalance == 0 || lastUpdateTimestamp == block.number
                || lastUpdateTimestamp >= DISTRIBUTION_END
        ) {
            return currentIndex;
        }

        uint256 currentTimestamp = block.number > DISTRIBUTION_END ? DISTRIBUTION_END : block.number;
