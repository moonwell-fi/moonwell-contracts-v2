...
contract MockMultichainGovernor is MultichainGovernor {
    ...
    function proposalValid(uint256 proposalId) external view returns (bool) {
        return proposalCount >= proposalId && proposalId > 0 && proposals[proposalId].proposer != address(0);
    }

    function userHasProposal(uint256 proposalId, address proposer) external view returns (bool) {
    ...
    function proposalInformationStruct(uint256 proposalId)
        external
        view
        returns (ProposalInformation memory proposalInfo)
    {
        ...
        proposalInfo.crossChainVoteCollectionEndTimestamp = proposal.crossChainVoteCollectionEndTimestamp;
