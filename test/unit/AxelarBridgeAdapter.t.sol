...
contract AxelarBridgeAdapterUnitTest is BaseTest {
    ...
    function setUp() public override {
        ...
        chainConfig = AxelarBridgeAdapter.ChainConfig({
            adapter: address(this),
            axelarid: axelarId
        });

        chainIds = AxelarBridgeAdapter.ChainIds({
    ...
    function testSetup() public {
        ...
        assertEq(
            adapter.chainIdToAxelarId(block.chainid),
            axelarId,
            "incorrect mapping of chain id to axelar id"
        );
        assertEq(
            adapter.axelarIdToChainId(axelarId),
            block.chainid,
            "incorrect mapping of axelar id to chain id"
        );
        assertEq(
            address(xwellProxy),
    ...
    function testaddExternalChainSendersNonOwnerFails() public {
        vm.expectRevert("Ownable: caller is not the owner");
        adapter.addExternalChainSenders(
            new AxelarBridgeAdapter.ChainConfig[](0)
        );
    }
    ...
    function testRemoveApprovedExternalChainSendersNonOwnerFails() public {
        vm.expectRevert("Ownable: caller is not the owner");
        adapter.removeExternalChainSenders(
            new AxelarBridgeAdapter.ChainConfig[](0)
        );
    }
    ...
    function testBridgeOutSuccess() public {
        ...
        assertEq(
            ...
            bridgeAmount,
            "invalid xwell amount after bridge out"
        );
    }
    ...
    function testBridgeInSuccess() public {
        ...
        assertEq(
            ...
            bridgeAmount,
            "invalid xwell amount after bridge in"
        );
    }
