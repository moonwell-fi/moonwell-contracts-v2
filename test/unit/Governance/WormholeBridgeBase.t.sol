...
contract WormholeBridgeBaseUnitTest is MultichainBaseTest {
    ...
    function testTrustedSenderInVoteCollectionFromWormholeFormat()
        public
        view
    {
        bytes32 trustedSenderBytes32 =
            bytes32(uint256(uint160(address(governor))));

        assertTrue(
            voteCollection.isTrustedSender(
                MOONBEAM_WORMHOLE_CHAIN_ID, trustedSenderBytes32
            ),
            "governor contract should be trusted sender from moonbeam"
        );
        ...
        address trustedSenderAddress =
            address(uint160(uint256(trustedSenderBytes32)));

        assertTrue(
            voteCollection.isTrustedSender(
                MOONBEAM_WORMHOLE_CHAIN_ID, trustedSenderAddress
            ),
    ...
    function testTrustedSenderInGovernorFromWormholeFormat() public view {
        bytes32 trustedSenderBytes32 =
            bytes32(uint256(uint160(address(voteCollection))));

        assertTrue(
            governor.isTrustedSender(
                BASE_WORMHOLE_CHAIN_ID, trustedSenderBytes32
            ),
            "vote collection contract should be trusted sender from base"
        );
        ...
        assertTrue(
            governor.isTrustedSender(
                BASE_WORMHOLE_CHAIN_ID, trustedSenderAddress
            ),
            "vote collection contract should be trusted sender from base"
        );
    }
    ...
    function _createProposal() private returns (uint256) {
        ...
        return governor.propose{value: bridgeCost}(
