...
contract MultichainMultipleVoteCollectionsUnitTest is MultichainBaseTest {
    ...
    function testEmitToMultipleVoteCollections() public returns (address proxyVoteCollection2) {
        ...
        emit BridgeOutSuccess(BASE_WORMHOLE_CHAIN_ID, bridgeCost / 2, address(voteCollection), payload);

        vm.expectEmit(true, true, true, true, address(governor));
        emit BridgeOutSuccess(chainId, bridgeCost / 2, proxyVoteCollection2, payload);

        governor.propose{value: bridgeCost}(targets, values, calldatas, description);

        {
            // vote collections should have the proposal
            (uint256 voteSnapshotTimestamp,,,,,,,) = voteCollection.proposalInformation(1);
            assertGt(voteSnapshotTimestamp, 0, "proposal id incorrect");
        }

        {
            MultichainVoteCollection voteCollection2 = MultichainVoteCollection(proxyVoteCollection2);
            (uint256 voteSnapshotTimestamp,,,,,,,) = voteCollection2.proposalInformation(1);
    ...
    function testEmitToMultipleVoteCollectionsSomeFails() public {
        ...
        WormholeTrustedSender.TrustedSender[] memory _trustedSenders = new WormholeTrustedSender.TrustedSender[](3);
        ...
        vm.expectEmit(true, true, true, true, address(governor));
        emit BridgeOutSuccess(BASE_WORMHOLE_CHAIN_ID, bridgeCost / 4, address(voteCollection), payload);
        ...
        {
            MultichainVoteCollection voteCollection3 = MultichainVoteCollection(proxyVoteCollection3);
            (uint256 voteSnapshotTimestamp,,,,,,,) = voteCollection3.proposalInformation(1);

            assertGt(voteSnapshotTimestamp, 0, "proposal doesn't exist");
        }

        {
            MultichainVoteCollection voteCollection4 = MultichainVoteCollection(proxyVoteCollection4);
            (uint256 voteSnapshotTimestamp,,,,,,,) = voteCollection4.proposalInformation(1);
    ...
    function testCollectVotesFromMultipleVoteCollections() public {
...

        {
            ...
            (uint256 totalVotes2, uint256 votesFor2, uint256 votesAgainst2, uint256 votesAbstain2) =
                voteCollection2.proposalVotes(1);

            assertEq(votesFor2, voteAmount, "votes for incorrect");
            assertEq(votesFor2 - votesForBefore2, voteAmount, "votes for incorrect");
            assertEq(votesAgainst2, votesAgainstBefore2, "votes against incorrect");
            assertEq(votesAbstain2, votesAbstainBefore2, "abstain votes incorrect");
            assertEq(totalVotes2, totalVotesBefore2 + voteAmount, "total votes incorrect");
        }
...

        {
...

            {
                ...
                vm.expectEmit(true, true, true, true, address(governor));
                emit CrossChainVoteCollected(proposalId, BASE_WORMHOLE_CHAIN_ID, 0, voteAmount, 0);
...

                {
                    ...
                    (uint256 forVotes, uint256 againstVotes, uint256 abstainVotes) =
                        governor.chainVoteCollectorVotes(BASE_WORMHOLE_CHAIN_ID, proposalId);
                ...
                assertEq(votesAgainst, voteAmount, "governor votes against incorrect");
                assertEq(votesFor, votesForBefore, "governor votes for incorrect");
                assertEq(votesAbstain, votesAbstainBefore, "governor abstain votes incorrect");
                assertEq(totalVotes, totalVotesBefore + votesAgainst, "governor total votes incorrect");
            }

            {
                uint256 bridgeCost = voteCollection2.bridgeCost(MOONBEAM_WORMHOLE_CHAIN_ID);
