...
contract MultichainVoteCollectionUnitTest is MultichainBaseTest {
    ...
    function testSetup() public view {
        ...
        assertEq(governor.gasLimit(), Constants.MIN_GAS_LIMIT, "incorrect gas limit vote collection");
        assertEq(voteCollection.gasLimit(), Constants.MIN_GAS_LIMIT, "incorrect gas limit vote collection");
        assertEq(
            voteCollection.getVotes(address(this), block.timestamp - 1), 4_000_000_000 * 1e18, "incorrect vote amount"
        );

        assertEq(address(voteCollection.xWell()), address(xwell), "xwell incorrect");
        assertEq(address(voteCollection.stkWell()), address(stkWellBase), "stkwell incorrect");
        ...
        assertEq(governor.getAllTargetChains().length, 1, "incorrect target chains length");
    ...
    function testProposeUpdateProposalThresholdSucceeds() public returns (uint256) {
        ...
        {
            IMultichainGovernor.ProposalInformation memory voteCollectionInfo =
                _getVoteCollectionProposalInformation(proposalId);

            IMultichainGovernor.ProposalInformation memory governorInfo = governor.proposalInformationStruct(proposalId);
            ...
            assertEq(voteCollectionInfo.votingStartTime, governorInfo.votingStartTime, "incorrect voting start time");
            assertEq(voteCollectionInfo.votingEndTime, governorInfo.votingEndTime, "incorrect end timestamp");
            assertEq(
    ...
    function testVotingValidProposalIdSucceeds() public returns (uint256 proposalId) {
        ...
        // get user vote power
        uint256 votePower = voteCollection.getVotes(address(this), voteSnapshotTimestamp);
        ...
        (bool hasVoted, uint256 voteValue, uint256 voteAmount) = voteCollection.getReceipt(proposalId, address(this));
        ...
        assertEq(totalVotes, totalVotesBefore + votePower, "total votes incorrect");
        assertEq(votesFor, totalVotes, "total votes incorrect");

        _assertGovernanceBalance();
    }

    function testVotingValidProposalIdBeforeStartFails() public returns (uint256 proposalId) {
        proposalId = _createProposalUpdateThreshold(address(this));

        (, uint256 votingStartTime,,,,,,) = voteCollection.proposalInformation(proposalId);
    ...
    function testVotingActiveProposalIdSucceeds() public returns (uint256 proposalId) {
        proposalId = _createProposalUpdateThreshold(address(this));
        ...
        assertEq(votesAgainst - votesAgainstBefore, 4_000_000_000 * 1e18, "votes against incorrect");
        assertEq(votesFor, votesForBefore, "votes for incorrect");
        assertEq(votesAbstain, votesAbstainBefore, "abstain votes incorrect");
        assertEq(totalVotes, totalVotesBefore + 4_000_000_000 * 1e18, "total votes incorrect");
        assertEq(votesAgainst, totalVotes, "total votes incorrect");

        _assertGovernanceBalance();
    }

    function testVotingPastVoteEndTimeProposalFails() public returns (uint256 proposalId) {
        proposalId = _createProposalUpdateThreshold(address(this));

        vm.warp(block.timestamp + governor.votingPeriod() + 1);

        assertEq(uint256(governor.state(proposalId)), 1, "incorrect state, not in crosschain vote collection period");
    ...
    function testMultipleUserVoteWellSucceeds() public {
...

        {
            ...
            assertEq(votesAgainst, votesAgainstBefore, "votes against incorrect");
            assertEq(votesAbstain, votesAbstainBefore, "abstain votes incorrect");
            assertEq(totalVotes, totalVotesBefore + voteAmount, "total votes incorrect");
        }

        {
            (bool hasVoted, uint8 voteValue, uint256 votes) = voteCollection.getReceipt(proposalId, user1);
...

        {
            ...
            assertEq(votesAgainst - votesAgainstBefore, voteAmount, "votes against incorrect");
            assertEq(votesFor, voteAmount, "votes for incorrect");
            assertEq(votesAbstain, votesAbstainBefore, "abstain votes incorrect");
            assertEq(totalVotes, totalVotesBefore + voteAmount * 2, "total votes incorrect");
        }

        {
            (bool hasVoted, uint8 voteValue, uint256 votes) = voteCollection.getReceipt(proposalId, user2);
...

        {
            ...
            assertEq(votesAbstain - votesAbstainBefore, voteAmount, "abstain votes incorrect");
            assertEq(votesFor, voteAmount, "votes for incorrect");
            assertEq(votesAgainst, voteAmount, "votes against incorrect");
            assertEq(totalVotes, totalVotesBefore + voteAmount * 3, "total votes incorrect");
        }

        {
            (bool hasVoted, uint8 voteValue, uint256 votes) = voteCollection.getReceipt(proposalId, user3);
...

        {
            ...
            assertEq(snapshotTimestamp, voteCollectionInfo.voteSnapshotTimestamp, "snapshot timestamp incorrect");
            assertEq(
            ...
            assertEq(
                ...
                voteCollectionInfo.forVotes + voteCollectionInfo.againstVotes + voteCollectionInfo.abstainVotes,
    ...
    function testMultipleUserVoteWithXWellDelegationSucceeds() public {
        ...
        {
            (bool hasVoted, uint8 voteValue, uint256 votes) = voteCollection.getReceipt(proposalId, user4);
        ...
        assertEq(voteCollectionInfo.totalVotes, 2 * voteAmount, "incorrect total votes");
        assertEq(voteCollectionInfo.forVotes, 0, "incorrect for votes");
        assertEq(voteCollectionInfo.againstVotes, voteAmount, "incorrect against votes");
        assertEq(voteCollectionInfo.abstainVotes, voteAmount, "incorrect abstain votes");
    ...
    function testMultipleUserVotexWellSucceeds() public {
        ...
        IMultichainGovernor.ProposalInformation memory governorInfo = _getVoteCollectionProposalInformation(proposalId);

        assertEq(
            governorInfo.totalVotes,
            governorInfo.forVotes + governorInfo.againstVotes + governorInfo.abstainVotes,
            "incorrect total votes"
        );

        assertEq(governorInfo.totalVotes, 3 * voteAmount, "incorrect total votes");
        assertEq(governorInfo.forVotes, voteAmount, "incorrect for votes");
        assertEq(governorInfo.againstVotes, voteAmount, "incorrect against votes");
        assertEq(governorInfo.abstainVotes, voteAmount, "incorrect abstain votes");
    ...
    function testMultipleUserVoteWithxWellDelegationSucceeds() public {
...

        {
            ...
            assertEq(votesFor, votesForBefore, "votes for incorrect");
            assertEq(votesAgainst - votesAgainstBefore, voteAmount, "votes against incorrect");
            assertEq(totalVotes, totalVotesBefore + voteAmount * 2, "total votes");
        }
    ...
    function testEmitVotesToGovernorSucceeded() public returns (uint256 proposalId) {
        testMultipleUserVoteWellSucceeds();

        proposalId = governor.proposalCount();

        IMultichainGovernor.ProposalInformation memory proposalVoteCollection =
            _getVoteCollectionProposalInformation(proposalId);
        ...
        IMultichainGovernor.ProposalInformation memory proposalBefore = governor.proposalInformationStruct(proposalId);

        {
            uint256 bridgeCost = voteCollection.bridgeCost(MOONBEAM_WORMHOLE_CHAIN_ID);
        ...
        IMultichainGovernor.ProposalInformation memory proposalAfter = governor.proposalInformationStruct(proposalId);
        ...
        assertEq(
            proposalAfter.forVotes, proposalBefore.forVotes + proposalVoteCollection.forVotes, "incorrect for votes"
        );
        assertEq(
    ...
    function testEmitVotesProposalEndTimeHasNotPassed() public {
        ...
        (,, uint256 endTimestamp,,,,,) = voteCollection.proposalInformation(proposalId);
    ...
    function testEmitVotesProposalEndTimeHasPassedBridgeOutIncorrectAmount() public {
        uint256 proposalId = testVotingValidProposalIdSucceeds();
        ...
        uint256 cost = voteCollection.bridgeCost(MOONBEAM_WORMHOLE_CHAIN_ID) - 1;
    ...
    function testSetStkWellOwnerSucceeds() public {
        ...
        assertEq(address(voteCollection.stkWell()), newstkWell, "stkwell not updated");
    }
    ...
    function testGetChainAddresVotes() public {
        ...
        (uint256 votesFor, uint256 votesAgainst, uint256 votesAbstain) = governor.chainAddressVotes(proposalId, 30); // base chain id
    ...
    function testBridgeInWrongPayloadLength() public {
        ...
        wormholeRelayerAdapter.sendPayloadToEvm{value: gasCost}(
            BASE_WORMHOLE_CHAIN_ID, address(voteCollection), payload, 0, 0
        );
    }
    ...
    function testBridgeInVotingSnapshotTimeGreaterThanStartTime() public {
        ...
        vm.deal(address(governor), gasCost);
        vm.prank(address(governor));
        vm.expectRevert("MultichainVoteCollection: snapshot time must be before start time");
        wormholeRelayerAdapter.sendPayloadToEvm{value: gasCost}(30, address(voteCollection), payload, 0, 0);
    }
    ...
    function testBridgeInVotingStartTimeGreaterThanVoteEndTime() public {
        ...
        vm.deal(address(governor), gasCost);
        vm.prank(address(governor));
        vm.expectRevert("MultichainVoteCollection: start time must be before end time");
        wormholeRelayerAdapter.sendPayloadToEvm{value: gasCost}(30, address(voteCollection), payload, 0, 0);
    }
    ...
    function testBridgeInVoteCollectionEndLtThanVoteEndTime() public {
        ...
        vm.deal(address(governor), gasCost);
        vm.prank(address(governor));
        vm.expectRevert("MultichainVoteCollection: end time must be before vote collection end");
        wormholeRelayerAdapter.sendPayloadToEvm{value: gasCost}(30, address(voteCollection), payload, 0, 0);
    }
    ...
    function testBridgeInVotingEndTimeLessThanTimestamp() public {
        ...
        vm.deal(address(governor), gasCost);
        vm.prank(address(governor));
        vm.expectRevert("MultichainVoteCollection: end time must be in the future");
        wormholeRelayerAdapter.sendPayloadToEvm{value: gasCost}(30, address(voteCollection), payload, 0, 0);
    }
