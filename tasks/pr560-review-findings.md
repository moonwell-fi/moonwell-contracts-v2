...
### M-2: Wrong proposalType in mips.json for mip-x53

- **File:** `proposals/mips/mips.json`
- **Issue:** `mip-x53` inherits `HybridProposalV2` but is registered as
  `"proposalType": "HybridProposal"`.
- **Fix:** Change to `"HybridProposalV2"` or verify tooling treats them
  equivalently.
- [ ] Fixed

### M-3: mip-x53 reads mTokens.json as proposal description

- **File:** `proposals/mips/mip-x53/mip-x53.sol:434-448`
- **Issue:** Proposal description is set to raw JSON content of `mTokens.json`
  instead of a human-readable description file.
- **Fix:** Read `MIP-X53.md` or a proper description file instead.
- [ ] Fixed
...
- [ ] Hardcoded `nonce = 2` in `mip-x53.sol:446` without coordination
- [ ] Missing `x41.md` description file (referenced at `mip-x41.sol:1293` but
      not in diff)
- [ ] `PAUSE_GUARDIAN` resolution in `mip-x53.sol:543-544` missing explicit
      chain context qualifier
- [ ] MIP-X53 mToken market init requires governor to hold real token balances
      on mainnet
