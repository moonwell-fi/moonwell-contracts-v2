...
/*
 ...
 Utility to deploy xWELL contract on testnet chains
...
*/
contract DeployXWellTestnet is Script, xWELLDeploy, Networks {
    ...
    /// the xWELL token
    uint256 public constant expectedNonce = 404;
    ...
    function run() public {
        ...
        if (!addresses.isAddressSet("xWELL_LOGIC")) {
            ...
            assertEq(
                xwellProxy,
                addresses.getAddress("xWELL_PROXY", BASE_SEPOLIA_CHAIN_ID),
                "xWELL_PROXY address is incorrect"
            );

            assertEq(
                xwellProxy,
                addresses.getAddress("xWELL_PROXY", MOONBASE_CHAIN_ID),
                "xWELL_PROXY address is incorrect"
            );
