...
output_title() {
  local text="# $1"
  echo "$text" > MutationTestOutput/Result_MultichainGovernor.md
}
...
\`\`\`\n$result\n\`\`\`
  ...
  echo "$content" >> MutationTestOutput/Result_MultichainGovernor.md
}
...
process_test_output() {
  ...
  echo "\n### $test_type:" >> MutationTestOutput/Result_MultichainGovernor.md
  ...
  if grep -q "Failing tests:" "$output"; then
    ...
    echo "Failed $test_type: $failed_tests, Passed Tests: $passed_tests" >> MutationTestOutput/Result_MultichainGovernor.md
    ...
    # Append to last_lines.txt with desired format
    echo "Failed $test_type: 0, Passed Tests: $passed_tests" >> MutationTestOutput/Result_MultichainGovernor.md
  fi
}
...
echo "$failed_mutation failed out of total $num_files through integration tests" >> MutationTestOutput/Result_MultichainGovernor.md