...
import {BASE_FORK_ID, ChainIds, MOONBEAM_FORK_ID, MOONBEAM_WORMHOLE_CHAIN_ID} from "@utils/ChainIds.sol";
...
contract mipm23 is Configs, HybridProposal, MultichainGovernorDeploy {
    ...
    constructor() {
        bytes memory proposalDescription = abi.encodePacked(vm.readFile("./src/proposals/mips/mip-m23/MIP-M23.md"));
    ...
    function build(Addresses addresses) public override {
        ...
        ITemporalGovernor.TrustedSender[] memory temporalGovernanceTrustedSenders =
        ...
        _pushAction(
            addresses.getAddress("TEMPORAL_GOVERNOR", block.chainid.toBaseChainId()),
        ...
        _pushAction(
            ...
            abi.encodeWithSignature(
                "setBufferCap(address,uint256)", addresses.getAddress("WORMHOLE_BRIDGE_ADAPTER_PROXY"), XWELL_BUFFER_CAP
            ),
        ...
        _pushAction(
            addresses.getAddress("WORMHOLE_BRIDGE_ADAPTER_PROXY"),
            abi.encodeWithSignature("transferOwnership(address)", multichainGovernorAddress),
        ...
        _pushAction(
            ...
            abi.encodeWithSignature("setAdmin(address)", multichainGovernorAddress),
        ...
        _pushAction(
            ...
            abi.encodeWithSignature("setEmissionsManager(address)", multichainGovernorAddress),
        ...
        {
            _setEmissionConfiguration("./src/proposals/mips/mip-m23/mip-m23.json");
        }
    ...
    function validate(Addresses addresses, address) public override {
        ...
        assertEq(Timelock(addresses.getAddress("MOONWELL_mETH")).admin(), timelock, "MOONWELL_mETH admin incorrect");

        assertEq(Timelock(addresses.getAddress("mxcUSDC")).pendingAdmin(), governor, "mxcUSDC pending admin incorrect");
        assertEq(Timelock(addresses.getAddress("mxcUSDC")).admin(), timelock, "mxcUSDC admin incorrect");

        assertEq(Timelock(addresses.getAddress("mUSDCwh")).pendingAdmin(), governor, "mUSDCwh pending admin incorrect");
        assertEq(Timelock(addresses.getAddress("mUSDCwh")).admin(), timelock, "mUSDCwh admin incorrect");

        assertEq(Timelock(addresses.getAddress("mFRAX")).pendingAdmin(), governor, "mFRAX pending admin incorrect");
        assertEq(Timelock(addresses.getAddress("mFRAX")).admin(), timelock, "mFRAX admin incorrect");

        assertEq(Timelock(addresses.getAddress("mxcUSDT")).pendingAdmin(), governor, "mxcUSDT pending admin incorrect");
        assertEq(Timelock(addresses.getAddress("mxcUSDT")).admin(), timelock, "mxcUSDT admin incorrect");

        assertEq(Timelock(addresses.getAddress("mxcDOT")).pendingAdmin(), governor, "mxcDOT pending admin incorrect");
        assertEq(Timelock(addresses.getAddress("mxcDOT")).admin(), timelock, "mxcDOT admin incorrect");

        assertEq(Timelock(addresses.getAddress("MNATIVE")).pendingAdmin(), governor, "MNATIVE pending admin incorrect");
        assertEq(Timelock(addresses.getAddress("MNATIVE")).admin(), timelock, "MNATIVE admin incorrect");
        ...
        assertEq(Timelock(addresses.getAddress("MOONWELL_mUSDC")).admin(), timelock, "MOONWELL_mUSDC admin incorrect");
        ...
        assertEq(Timelock(addresses.getAddress("MOONWELL_mBUSD")).admin(), timelock, "MOONWELL_mBUSD admin incorrect");
        ...
        assertEq(
            Timelock(addresses.getAddress("UNITROLLER")).pendingAdmin(), governor, "UNITROLLER pending admin incorrect"
        );
        assertEq(Timelock(addresses.getAddress("UNITROLLER")).admin(), timelock, "UNITROLLER admin incorrect");

        assertEq(
            Timelock(addresses.getAddress("CHAINLINK_ORACLE")).admin(), governor, "Chainlink oracle admin incorrect"
        );

        assertEq(
            xWELL(addresses.getAddress("xWELL_PROXY")).bufferCap(addresses.getAddress("WORMHOLE_BRIDGE_ADAPTER_PROXY")),
            XWELL_BUFFER_CAP,
        ...
        TemporalGovernor temporalGovernor = TemporalGovernor(payable(addresses.getAddress("TEMPORAL_GOVERNOR")));
