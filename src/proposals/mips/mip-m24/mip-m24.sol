...
contract mipm24 is HybridProposal, MultichainGovernorDeploy {
    ...
    constructor() {
        bytes memory proposalDescription = abi.encodePacked(vm.readFile("./src/proposals/mips/mip-m24/MIP-M24.md"));
    ...
    function build(Addresses addresses) public override {
        ITemporalGovernor.TrustedSender[] memory trustedSendersToRemove = new ITemporalGovernor.TrustedSender[](1);

        trustedSendersToRemove[0].addr = addresses.getAddress("MOONBEAM_TIMELOCK");
        ...
        _pushAction(
            ...
            abi.encodeWithSignature("unSetTrustedSenders((uint16,address)[])", trustedSendersToRemove),
    ...
    function validate(Addresses addresses, address) public override {
        ...
        assertEq(Timelock(addresses.getAddress("MOONWELL_mETH")).admin(), governor, "MOONWELL_mETH admin incorrect");
        assertEq(
        ...
        assertEq(
            Timelock(addresses.getAddress("mxcUSDC")).pendingAdmin(), address(0), "mxcUSDC pending admin incorrect"
        );
        assertEq(Timelock(addresses.getAddress("mxcUSDC")).admin(), governor, "mxcUSDC admin incorrect");

        assertEq(
            Timelock(addresses.getAddress("mUSDCwh")).pendingAdmin(), address(0), "mUSDCwh pending admin incorrect"
        );
        assertEq(Timelock(addresses.getAddress("mUSDCwh")).admin(), governor, "mUSDCwh admin incorrect");

        assertEq(Timelock(addresses.getAddress("mFRAX")).admin(), governor, "mFRAX admin incorrect");
        assertEq(Timelock(addresses.getAddress("mFRAX")).pendingAdmin(), address(0), "mFRAX pending admin incorrect");

        assertEq(
            Timelock(addresses.getAddress("mxcUSDT")).pendingAdmin(), address(0), "mxcUSDT pending admin incorrect"
        );
        assertEq(Timelock(addresses.getAddress("mxcUSDT")).admin(), governor, "mxcUSDT admin incorrect");

        assertEq(Timelock(addresses.getAddress("mxcDOT")).pendingAdmin(), address(0), "mxcDOT pending admin incorrect");
        assertEq(Timelock(addresses.getAddress("mxcDOT")).admin(), governor, "mxcDOT admin incorrect");

        assertEq(
            Timelock(addresses.getAddress("MNATIVE")).pendingAdmin(), address(0), "MNATIVE pending admin incorrect"
        );
        assertEq(Timelock(addresses.getAddress("MNATIVE")).admin(), governor, "MNATIVE admin incorrect");
        ...
        assertEq(Timelock(addresses.getAddress("MOONWELL_mUSDC")).admin(), governor, "MOONWELL_mUSDC admin incorrect");
        ...
        assertEq(Timelock(addresses.getAddress("MOONWELL_mBUSD")).admin(), governor, "MOONWELL_mBUSD admin incorrect");
        ...
        assertEq(Timelock(addresses.getAddress("UNITROLLER")).admin(), governor, "UNITROLLER admin incorrect");
        ...
        bytes32[] memory trustedSenders = temporalGovernor.allTrustedSenders(block.chainid.toMoonbeamWormholeChainId());
