...
function validateProxy(Vm vm, address proxy, address logic, address admin, string memory error) view {
    {
        bytes32 data = vm.load(proxy, _ADMIN_SLOT);

        require(bytes32(uint256(uint160(admin))) == data, string(abi.encodePacked(error, " admin not set correctly")));
    }
