...
abstract contract Proposal is Script, Test {
    ...
    function run() public virtual {
        ...
        if (DO_PRE_BUILD_MOCK) {
            preBuildMock(addresses);
        }
        if (DO_BUILD) {
            build(addresses);
        }
        if (DO_RUN) {
            run(addresses, deployerAddress);
        }
        if (DO_TEARDOWN) {
            teardown(addresses, deployerAddress);
        }
        if (DO_VALIDATE) {
    ...
    function getProposalId(Addresses, address) public virtual returns (uint256 proposalId);
