...
contract WormholeUnwrapperAdapter is WormholeBridgeAdapter {
    ...
    function setLockbox(address _lockbox) external onlyOwner {
        require(lockbox == address(0), "WormholeUnwrapperAdapter: lockbox already set");
        lockbox = _lockbox;
    ...
    function _bridgeIn(uint256 chainId, address user, uint256 amount) internal override {
