...
contract BaseMoonwellViews is Initializable {
    ...
    function initialize(
        ...
        require(_comptroller != address(0), "Comptroller cant be the 0 address!");

        comptroller = Comptroller(payable(_comptroller));

        require(comptroller.isComptroller(), "Cant bind to something thats not a comptroller!");

        _tokenSaleDistributor = TokenSaleDistributorInterfaceV1(address(tokenSaleDistributor));
    ...
    function _getSupplyCaps(address _market) internal view virtual returns (uint256) {}

    /// @notice A view to get a specific market info
    function getMarketInfo(MToken _mToken) external view returns (Market memory) {
        Market memory _result;

        (bool _isListed, uint256 _collateralFactor) = comptroller.markets(address(_mToken));
        ...
        if (_isListed) {
            ...
            _result.mintPaused = comptroller.mintGuardianPaused(address(_mToken));
            _result.borrowPaused = comptroller.borrowGuardianPaused(address(_mToken));
            _result.underlyingPrice = comptroller.oracle().getUnderlyingPrice(_mToken);
    ...
    function getMarketsInfo(MToken[] calldata _mTokens) external view returns (Market[] memory) {
    ...
    function getStakingInfo() external view returns (StakingInfo memory _result) {
        if (address(safetyModule) != address(0)) {
            ...
            SafetyModuleInterfaceV1.AssetData memory asset = safetyModule.assets(address(safetyModule));
    ...
    function getMarketIncentives(MToken market) public view virtual returns (MarketIncentives[] memory) {}

    /// @notice A view to get the user voting power from the tokens staking in the safety module
    function getUserStakingVotingPower(address _user) public view virtual returns (Votes memory _result) {
        if (address(safetyModule) != address(0)) {
            uint256 _priorVotes = safetyModule.getPriorVotes(_user, block.number - 1);
    ...
    function getUserClaimsVotingPower(address _user) public view virtual returns (Votes memory _result) {
        if (address(_tokenSaleDistributor) != address(0)) {
            uint256 _priorVotes = _tokenSaleDistributor.getPriorVotes(_user, block.number - 1);
            uint256 _totalAllocated = _tokenSaleDistributor.totalAllocated(_user);
            uint256 _totalClaimed = _tokenSaleDistributor.totalClaimed(_user);
            address _delegates = _tokenSaleDistributor.delegates(_user);
            _result = Votes(_priorVotes, (_totalAllocated - _totalClaimed), _delegates);
        }
    }

    /// @notice A view to get the user voting power from the user holdings
    function getUserTokensVotingPower(address _user) public view virtual returns (Votes memory _result) {
        if (address(governanceToken) != address(0)) {
            uint256 _priorVotes = governanceToken.getPriorVotes(_user, block.number - 1);
            address _delegates = governanceToken.delegates(_user);
            _result = Votes(_priorVotes, governanceToken.balanceOf(_user), _delegates);
        }
    }

    /// @notice A view to get the user voting power from all the possible sources
    function getUserVotingPower(address _user) public view virtual returns (UserVotes memory _result) {
    ...
    function getTokensBalances(address[] memory _tokens, address _user) public view returns (Balances[] memory) {
        ...
        for (uint256 index = 0; index < _tokens.length; index++) {
            if (_tokens[index] == address(0)) {
                ...
                _result[index] = Balances(token.balanceOf(_user), address(token));
            }
        }
    ...
    function getUserBalances(address _user) public view returns (Balances[] memory) {
        ...
        for (uint256 index = 0; index < _mTokens.length; index++) {
            ...
            if (address(mToken) != _nativeMarket) {
                underlyingToken = address(MErc20Interface(address(mToken)).underlying());
            }
    ...
    function getUserBorrowsBalances(address _user) public view returns (Balances[] memory) {
        MToken[] memory _mTokens = comptroller.getAllMarkets();
        ...
        for (uint256 index = 0; index < _mTokens.length; index++) {
            MToken mToken = _mTokens[index];
            _result[index] = Balances(mToken.borrowBalanceStored(_user), address(mToken));
        }
    ...
    function getUserMarketsMemberships(address _user) public view returns (Memberships[] memory) {
        MToken[] memory _mTokens = comptroller.getAllMarkets();
        ...
        for (uint256 index = 0; index < _mTokens.length; index++) {
            MToken mToken = _mTokens[index];
            _result[index] = Memberships(comptroller.checkMembership(_user, mToken), address(mToken));
        }
    ...
    /// @notice Virtual function to get the user accrued and pendings rewards, must be overriden depending on the version of the deployment
    function getUserRewards(address _user) public view virtual returns (Rewards[] memory) {}

    /// @notice Virtual function to get the user staking info
    function getUserStakingInfo(address _user) public view returns (UserStakingInfo memory _result) {
        if (address(safetyModule) != address(0)) {
    ...
    function getProtocolInfo() public view returns (ProtocolInfo memory _result) {
    ...
    function getNativeTokenPrice() public view returns (uint256 _result) {
        if (address(_nativeMarket) != address(0)) {
            _result = comptroller.oracle().getUnderlyingPrice(MToken(_nativeMarket));
        }
    }

    /// @notice Function to get the governance token price
    function getGovernanceTokenPrice() public view returns (uint256 _result) {
        if (address(_governanceTokenLP) != address(0) && address(_nativeMarket) != address(0)) {
            (uint256 reserves0, uint256 reserves1,) = _governanceTokenLP.getReserves();
            address token0 = _governanceTokenLP.token0();

            uint256 _nativeReserve = token0 == address(governanceToken) ? reserves1 : reserves0;
            uint256 _tokenReserve = token0 == address(governanceToken) ? reserves0 : reserves1;
