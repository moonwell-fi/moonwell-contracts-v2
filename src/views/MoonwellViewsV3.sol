...
contract MoonwellViewsV3 is BaseMoonwellViews {
    ...
    function getMarketIncentives(MToken market) public view override returns (MarketIncentives[] memory) {
        ...
        MarketIncentives[] memory _result = new MarketIncentives[](_indexHelper);
    ...
    function getUserTokensVotingPower(address _user) public view override returns (Votes memory _result) {
        if (address(governanceToken) != address(0)) {
            uint256 _priorVotes = xWELL(address(governanceToken)).getVotes(_user);
    ...
    function getUserStakingVotingPower(address _user) public view override returns (Votes memory _result) {
        if (address(safetyModule) != address(0)) {
            uint256 _priorVotes =
                SafetyModuleInterfaceV1(address(safetyModule)).getPriorVotes(_user, block.timestamp - 1);
