...
import {ComptrollerInterfaceV1} from
    "@protocol/views/ComptrollerInterfaceV1.sol";
...
contract MoonwellViewsV1 is BaseMoonwellViews, ExponentialNoError {
    ...
    {
        ComptrollerInterfaceV1 comptrollerV1 =
            ComptrollerInterfaceV1(address(comptroller));
        ...
        MarketIncentives[] memory _result =
            new MarketIncentives[](_incentives.length);
        ...
        returns (ComptrollerInterfaceV1.RewardMarketState memory _result)
    {
        ...
        Exp memory marketBorrowIndex =
            Exp({mantissa: MToken(mToken).borrowIndex()});

        ComptrollerInterfaceV1.RewardMarketState memory borrowState =
            comptroller.rewardBorrowState(rewardType, mToken);
        uint256 borrowSpeed = comptroller.borrowRewardSpeeds(rewardType, mToken);
        uint256 blockTimestamp = block.timestamp;
        uint256 deltaTimestamps =
            sub_(blockTimestamp, uint256(borrowState.timestamp));

        if (deltaTimestamps > 0 && borrowSpeed > 0) {
            uint256 borrowAmount =
                div_(MToken(mToken).totalBorrows(), marketBorrowIndex);
    ...
    function _getSupplierReward(
        ...
        Double memory supplierIndex = Double({
            mantissa: comptroller.rewardSupplierIndex(
                rewardType, address(mToken), supplier
            )
        });
    ...
    function _getBorrowerReward(
        ...
        Double memory borrowerIndex = Double({
            mantissa: comptroller.rewardBorrowerIndex(
                rewardType, address(mToken), borrower
            )
        });
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            uint256 borrowerAmount =
                div_(mToken.borrowBalanceStored(borrower), marketBorrowIndex);
        ...
        returns (Rewards[] memory)
    {
        ...
        for (uint256 i = 0; i < _mTokens.length; i++) {
            ...
            _result[_currIndex].rewardToken =
                address(comptrollerV1.wellAddress());
            ...
            if (comptrollerV1.markets(address(mToken)).isListed) {
                _result[_currIndex].supplyRewardsAmount =
                    _getSupplierReward(comptrollerV1, 0, mToken, _user);
                _result[_currIndex].borrowRewardsAmount =
                    _getBorrowerReward(comptrollerV1, 0, mToken, _user);

                _result[_currIndex + 1].supplyRewardsAmount =
                    _getSupplierReward(comptrollerV1, 1, mToken, _user);
                _result[_currIndex + 1].borrowRewardsAmount =
                    _getBorrowerReward(comptrollerV1, 1, mToken, _user);

                if (_distributedAccrued == false) {
                    _result[_currIndex].supplyRewardsAmount +=
                        comptrollerV1.rewardAccrued(0, _user);
                    _result[_currIndex + 1].supplyRewardsAmount +=
                        comptrollerV1.rewardAccrued(1, _user);
