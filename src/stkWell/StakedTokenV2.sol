 ...
 * @dev Upgraded contract to use ERC20WithSnapshotV2
 * @author Moonwell
 **/
    ...
    ReentrancyGuardUpgradeableV2
{
    ...
    /// @notice default timestamp to use for snapshots where there is value but no timestamp (ie v1 stakers)
    uint256 public defaultSnapshotTimestamp;
    ...
    function __StakedToken_initV2() internal reinitializer(2) {
        // Set default snapshot timestamp to the upgrade time so that old V1 stakers
        // (who have snapshots with timestamp=0) can vote on all proposals created after upgrade.
        // Old snapshots with timestamp=0 will be treated as having this default timestamp.
