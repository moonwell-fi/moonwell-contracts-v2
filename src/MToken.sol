...
abstract contract MToken is MTokenInterface, Exponential, TokenErrorReporter {
    ...
    function initialize(
        ...
        require(
            err == uint256(Error.NO_ERROR), "setting interest rate model failed"
        );
     ...
     */
    function borrowRatePerTimestamp()
        external
        view
        override
        returns (uint256)
     ...
     */
    function supplyRatePerTimestamp()
        external
        view
        override
        returns (uint256)
        ...
        returns (MathError, uint256)
    {
         ...
         */
        (mathErr, principalTimesIndex) =
            mulUInt(borrowSnapshot.principal, borrowIndex);
        if (mathErr != MathError.NO_ERROR) {
            return (mathErr, 0);
        }

        (mathErr, result) =
            divUInt(principalTimesIndex, borrowSnapshot.interestIndex);
        if (mathErr != MathError.NO_ERROR) {
            return (mathErr, 0);
        }
    ...
    function accrueInterest() public virtual override returns (uint256) {
        ...
        (MathError mathErr, uint256 blockDelta) =
            subUInt(currentBlockTimestamp, accrualBlockTimestampPrior);
        require(
            mathErr == MathError.NO_ERROR, "could not calculate block delta"
        );
        ...
        (mathErr, interestAccumulated) =
            mulScalarTruncate(simpleInterestFactor, borrowsPrior);
        if (mathErr != MathError.NO_ERROR) {
            return failOpaque(
                Error.MATH_ERROR,
                FailureInfo
        ...
        returns (uint256, uint256)
    {
        ...
        if (error != uint256(Error.NO_ERROR)) {
            ...
            return
                (fail(Error(error), FailureInfo.MINT_ACCRUE_INTEREST_FAILED), 0);
        }
        ...
        returns (uint256, uint256)
    {
        ...
        (vars.mathErr, vars.mintTokens) = divScalarByExpTruncate(
            vars.actualMintAmount, Exp({mantissa: vars.exchangeRateMantissa})
        );
        require(
        ...
        (vars.mathErr, vars.totalSupplyNew) =
            addUInt(totalSupply, vars.mintTokens);
        require(
            vars.mathErr == MathError.NO_ERROR,
            "MINT_NEW_TOTAL_SUPPLY_CALCULATION_FAILED"
        );

        (vars.mathErr, vars.accountTokensNew) =
            addUInt(accountTokens[minter], vars.mintTokens);
        require(
            vars.mathErr == MathError.NO_ERROR,
    ...
    function redeemFresh(
        ...
        if (redeemTokensIn > 0) {
            ...
            (vars.mathErr, vars.redeemAmount) = mulScalarTruncate(
                Exp({mantissa: vars.exchangeRateMantissa}), vars.redeemTokens
            );
            if (vars.mathErr != MathError.NO_ERROR) {
                return failOpaque(
                    Error.MATH_ERROR,
            ...
            if (redeemAmountIn == type(uint256).max) {
                ...
                (vars.mathErr, vars.redeemTokens) = divScalarByExpTruncate(
                    redeemAmountIn, Exp({mantissa: vars.exchangeRateMantissa})
                );
                if (vars.mathErr != MathError.NO_ERROR) {
                    return failOpaque(
                        Error.MATH_ERROR,
        ...
        uint256 allowed = comptroller.redeemAllowed(
            address(this), redeemer, vars.redeemTokens
        );
        if (allowed != 0) {
            return failOpaque(
                Error.COMPTROLLER_REJECTION,
        ...
        (vars.mathErr, vars.accountTokensNew) =
            subUInt(accountTokens[redeemer], vars.redeemTokens);
        if (vars.mathErr != MathError.NO_ERROR) {
            return failOpaque(
                Error.MATH_ERROR,
        ...
        comptroller.redeemVerify(
            address(this), redeemer, vars.redeemAmount, vars.redeemTokens
        );
        ...
        returns (uint256)
    {
         ...
         */
        (vars.mathErr, vars.accountBorrows) =
        ...
        (vars.mathErr, vars.accountBorrowsNew) =
            addUInt(vars.accountBorrows, borrowAmount);
        if (vars.mathErr != MathError.NO_ERROR) {
            return failOpaque(
                Error.MATH_ERROR,
        ...
        (vars.mathErr, vars.totalBorrowsNew) =
    ...
    function repayBorrowFresh(
        ...
        uint256 allowed = comptroller.repayBorrowAllowed(
            address(this), payer, borrower, repayAmount
        );
        if (allowed != 0) {
            return (
                failOpaque(
         ...
         */
        (vars.mathErr, vars.accountBorrowsNew) =
            subUInt(vars.accountBorrows, vars.actualRepayAmount);
        require(
            vars.mathErr == MathError.NO_ERROR,
        ...
        (vars.mathErr, vars.totalBorrowsNew) =
            subUInt(totalBorrows, vars.actualRepayAmount);
        require(
            vars.mathErr == MathError.NO_ERROR,
    ...
    function liquidateBorrowInternal(
        ...
        return liquidateBorrowFresh(
            msg.sender, borrower, repayAmount, mTokenCollateral
        );
    }
    ...
    function liquidateBorrowFresh(
        ...
        (uint256 amountSeizeError, uint256 seizeTokens) = comptroller
            ...
            address(this), address(mTokenCollateral), actualRepayAmount
        );
        require(
    ...
    function seizeInternal(
         ...
         */
        (vars.mathErr, vars.borrowerTokensNew) =
        ...
        vars.protocolSeizeAmount = mul_ScalarTruncate(
            Exp({mantissa: vars.exchangeRateMantissa}), vars.protocolSeizeTokens
        );

        vars.totalReservesNew = add_(totalReserves, vars.protocolSeizeAmount);
        vars.totalSupplyNew = sub_(totalSupply, vars.protocolSeizeTokens);

        (vars.mathErr, vars.liquidatorTokensNew) =
            addUInt(accountTokens[liquidator], vars.liquidatorSeizeTokens);
        if (vars.mathErr != MathError.NO_ERROR) {
            return failOpaque(
                Error.MATH_ERROR,
        ...
        emit ReservesAdded(
            address(this), vars.protocolSeizeAmount, vars.totalReservesNew
        );
        ...
        returns (uint256)
    {
        ...
        if (msg.sender != admin) {
            return fail(
                Error.UNAUTHORIZED, FailureInfo.SET_COMPTROLLER_OWNER_CHECK
            );
        }
        ...
        returns (uint256)
    {
        ...
        if (newReserveFactorMantissa > reserveFactorMaxMantissa) {
            return fail(
                Error.BAD_INPUT, FailureInfo.SET_RESERVE_FACTOR_BOUNDS_CHECK
            );
        }
        ...
        returns (uint256)
    {
        ...
        if (error != uint256(Error.NO_ERROR)) {
            ...
            return fail(
                Error(error), FailureInfo.ADD_RESERVES_ACCRUE_INTEREST_FAILED
            );
        }
        ...
        returns (uint256, uint256)
    {
        ...
        if (accrualBlockTimestamp != getBlockTimestamp()) {
            return (
                fail(
                    Error.MARKET_NOT_FRESH, FailureInfo.ADD_RESERVES_FRESH_CHECK
                ),
                actualAddAmount
            );
        }
        ...
        returns (uint256)
    {
        ...
        if (error != uint256(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but on top of that we want to log the fact that an attempted reduce reserves failed.
            return fail(
                Error(error), FailureInfo.REDUCE_RESERVES_ACCRUE_INTEREST_FAILED
            );
        }
        ...
        returns (uint256)
    {
        ...
        if (msg.sender != admin) {
            return fail(
                Error.UNAUTHORIZED, FailureInfo.REDUCE_RESERVES_ADMIN_CHECK
            );
        }
        ...
        if (accrualBlockTimestamp != getBlockTimestamp()) {
            return fail(
                Error.MARKET_NOT_FRESH, FailureInfo.REDUCE_RESERVES_FRESH_CHECK
            );
        }
