...
import {IWormholeTrustedSender} from
    "@protocol/governance/IWormholeTrustedSender.sol";
...
contract WormholeTrustedSender is IWormholeTrustedSender {
    ...
    function _addTrustedSenders(TrustedSender[] memory _trustedSenders)
        internal
    {
        unchecked {
            for (uint256 i = 0; i < _trustedSenders.length; i++) {
                _addTrustedSender(
                    _trustedSenders[i].addr, _trustedSenders[i].chainId
                );
            }
        }
    }
    ...
    function _addTrustedSender(address trustedSender, uint16 chainId)
        internal
    {
        require(
    ...
    function _removeTrustedSender(address trustedSender, uint16 chainId)
        internal
    {
        require(
    ...
    function _removeTrustedSenders(TrustedSender[] memory _trustedSenders)
        internal
    {
        unchecked {
            for (uint256 i = 0; i < _trustedSenders.length; i++) {
