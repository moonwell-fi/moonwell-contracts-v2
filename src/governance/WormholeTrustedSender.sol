...
contract WormholeTrustedSender is IWormholeTrustedSender {
    ...
    function _addTrustedSenders(TrustedSender[] memory _trustedSenders) internal {
        unchecked {
            for (uint256 i = 0; i < _trustedSenders.length; i++) {
                _addTrustedSender(_trustedSenders[i].addr, _trustedSenders[i].chainId);
            }
        }
    }
    ...
    function _addTrustedSender(address trustedSender, uint16 chainId) internal {
        require(trustedSenders[chainId].add(addressToBytes(trustedSender)), "WormholeTrustedSender: already in list");
    ...
    function _removeTrustedSender(address trustedSender, uint16 chainId) internal {
        require(trustedSenders[chainId].remove(addressToBytes(trustedSender)), "WormholeTrustedSender: not in list");
    ...
    function _removeTrustedSenders(TrustedSender[] memory _trustedSenders) internal {
        unchecked {
            for (uint256 i = 0; i < _trustedSenders.length; i++) {
                _removeTrustedSender(_trustedSenders[i].addr, _trustedSenders[i].chainId);
            }
        }
    }
    ...
    function isTrustedSender(uint16 chainId, bytes32 addr) public view override returns (bool) {
    ...
    /// @param chainId The wormhole chain id to check
    /// @param addr The address to check
    function isTrustedSender(uint16 chainId, address addr) public view override returns (bool) {
