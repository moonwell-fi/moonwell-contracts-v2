...
contract MultichainGovernorDeploy is Test {
    function deployMultichainGovernor(address proxyAdmin) public returns (address proxy, address governorImpl) {
        ...
        proxy = address(new TransparentUpgradeableProxy(governorImpl, proxyAdmin, ""));
    }

    function deployMockMultichainGovernor(address proxyAdmin) public returns (address proxy, address governorImpl) {
    ...
    function initializeMultichainGovernor(
        ...
        MultichainGovernor(governorProxy).initialize(initializeData, trustedSenders, whitelistedCalldata);
    }
    ...
    function deployVoteCollection(
        ...
        proxy = address(new TransparentUpgradeableProxy(voteCollectionImpl, proxyAdmin, initData));
    }
    ...
    function deployGovernorRelayerAndVoteCollection(
        ...
        proxyAdmin = proxyAdmin == address(0) ? address(new ProxyAdmin()) : proxyAdmin;

        // deploy governor
        (address gProxy, address gImplementation) = deployMockMultichainGovernor(proxyAdmin);
        ...
        trustedSenders[0] = WormholeTrustedSender.TrustedSender({chainId: baseChainId, addr: vProxy});

        /// add wormhole relayer adapter to initialize function
        initializeData.wormholeRelayer = wormholeRelayerAdapter;

        initializeMultichainGovernor(gProxy, initializeData, trustedSenders, whitelistedCalldata);
    ...
    function deployStakedWellMock(
        ...
        implementation = deployCode("artifacts/foundry/MockStakedWell.sol/MockStakedWell.json");
    ...
    function deployStakedWell(
        ...
        implementation = deployCode("artifacts/foundry/StakedWell.sol/StakedWell.json");
    ...
    function deployEcosystemReserve(address proxyAdmin)
        public
        returns (
    ...
    {
        ecosystemReserveImplementation = deployCode("artifacts/foundry/EcosystemReserve.sol/EcosystemReserve.json");

        ecosystemReserveController =
            deployCode("artifacts/foundry/EcosystemReserveController.sol/EcosystemReserveController.json");

        ecosystemReserveProxy = address(
            new TransparentUpgradeableProxy(
                ecosystemReserveImplementation,
                proxyAdmin,
                abi.encodeWithSignature("initialize(address)", ecosystemReserveController)
            )
        );
    }
}
