...
contract MultichainGovernor is IMultichainGovernor, ConfigurablePauseGuardian, WormholeBridgeBase {
    ...
    mapping(address user => EnumerableSet.UintSet userProposals) internal _userLiveProposals;

    /// @notice the number of votes for a given proposal on a given chainid
    mapping(uint16 wormholeChainId => mapping(uint256 proposalId => VoteCounts)) public chainVoteCollectorVotes;
        ...
        )
    {
        ...
        crossChainVoteCollectionEndTimestamp = proposal.crossChainVoteCollectionEndTimestamp;
    ...
    function proposalVotes(uint256 proposalId)
        external
        view
        returns (uint256 totalVotes, uint256 forVotes, uint256 againstVotes, uint256 abstainVotes)
    {
        Proposal storage proposal = proposals[proposalId];
    ...
    /// @param user The address of the user to check
    function getUserLiveProposals(address user) external view returns (uint256[] memory) {
        uint256[] memory userProposals = new uint256[](currentUserLiveProposals(user));
    ...
    function getVotes(address account, uint256 timestamp, uint256 blockNumber) public view returns (uint256) {
        ...
        uint256 stkWellVotes = stkWell.getPriorVotes(account, useTimestamps ? timestamp : blockNumber);
    ...
    function proposalActive(uint256 proposalId) public view returns (bool) {
        ...
        return proposalState == ProposalState.Active || proposalState == ProposalState.CrossChainVoteCollection;
    }
    ...
    function chainAddressVotes(
        ...
        VoteCounts storage voteCounts = chainVoteCollectorVotes[wormholeChainId][proposalId];
    ...
    function state(uint256 proposalId) public view returns (ProposalState) {
        ...
        } else if (block.timestamp <= proposal.crossChainVoteCollectionEndTimestamp) {
    ...
    function rebroadcastProposal(uint256 proposalId) external payable {
        ProposalState proposalState = state(proposalId);
        require(proposalState == ProposalState.Active, "MultichainGovernor: invalid state");
    ...
    function propose(
        ...
        /// remove inactive proposals from all proposals, and remove from inactive proposals from user list
...

        {
            ...
            payload = abi.encode(
                proposalCount, voteSnapshotTimestamp, startTimestamp, endTimestamp, crossChainVoteCollectionEndTimestamp
            );

            emit ProposalCreated(
                proposalCount, msg.sender, targets, values, calldatas, startTimestamp, endTimestamp, description
            );
        }
        ...
        require(
            _liveProposals.add(proposalCount), "MultichainGovernor: cannot add the same proposal twice to global set"
        );
    ...
    function cancel(uint256 proposalId) external override {
        require(
            msg.sender == proposals[proposalId].proposer
                || getCurrentVotes(proposals[proposalId].proposer) < proposalThreshold,
            "MultichainGovernor: unauthorized cancel"
        );

        ProposalState proposalState = state(proposalId);

        require(proposalState == ProposalState.Active, "MultichainGovernor: cannot cancel non active proposal");
    ...
    function castVote(uint256 proposalId, uint8 voteValue) external override whenNotPaused {
        require(state(proposalId) == ProposalState.Active, "MultichainGovernor: proposal not active");
        ...
        /// function.
        uint256 votes = getVotes(msg.sender, proposal.voteSnapshotTimestamp, proposal.voteSnapshotBlock);
    ...
    function setNewStakedWell(address newStakedWell, bool toUseTimestamps) external onlyGovernor {
    ...
    function removeExternalChainConfigs(WormholeTrustedSender.TrustedSender[] memory _trustedSenders)
        external
        onlyGovernor
    {
    ...
    function addExternalChainConfigs(WormholeTrustedSender.TrustedSender[] memory _trustedSenders)
        external
        onlyGovernor
    {
    ...
    function setGasLimit(uint96 newGasLimit) external onlyGovernor {
        require(newGasLimit >= Constants.MIN_GAS_LIMIT, "MultichainGovernor: gas limit too low");
    ...
    function grantPauseGuardian(address newPauseGuardian) external onlyGovernor whenNotPaused {
    ...
    function executeBreakGlass(
        ...
        require(targets.length == calldatas.length, "MultichainGovernor: arity mismatch");
        ...
        unchecked {
            for (uint256 i = 0; i < calldatas.length; i++) {
                ...
                targets[i].functionCall(calldatas[i], "MultichainGovernor: break glass guardian call failed");
            }
        }

        require(breakGlassGuardian == address(0), "MultichainGovernor: break glass guardian not null");
    ...
    function pause() public override {
        ...
        for (uint256 i = 0; i < allProposals.length;) {
            ...
            ///
            if (
                proposalState != ProposalState.Executed && proposalState != ProposalState.Defeated
    ...
    /// @param data the calldata to update approval for
    /// @param approved whether or not the calldata is approved
    function _updateApprovedCalldata(bytes calldata data, bool approved) private {
        ...
        if (approved == true) {
            ...
            require(whitelistedCalldatas[data], "MultichainGovernor: calldata not approved");
        }
    ...
    function _setCrossChainVoteCollectionPeriod(uint256 _crossChainVoteCollectionPeriod) private {
        require(
            _crossChainVoteCollectionPeriod >= Constants.MIN_CROSS_CHAIN_VOTE_COLLECTION_PERIOD
                && _crossChainVoteCollectionPeriod <= Constants.MAX_CROSS_CHAIN_VOTE_COLLECTION_PERIOD,
    ...
    function _setMaxUserLiveProposals(uint256 _maxUserLiveProposals) private {
        require(
            _maxUserLiveProposals != 0 && _maxUserLiveProposals <= Constants.MAX_USER_PROPOSAL_COUNT,
    ...
    function _setQuorum(uint256 _quorum) private {
        require(_quorum <= Constants.MAX_QUORUM, "MultichainGovernor: invalid quorum");
    ...
    function _setVotingPeriod(uint256 _votingPeriod) private {
        require(
            _votingPeriod >= Constants.MIN_VOTING_PERIOD && _votingPeriod <= Constants.MAX_VOTING_PERIOD,
    ...
    function _bridgeIn(uint16 sourceChain, bytes memory payload) internal override {
        ...
        VoteCounts storage voteCounts = chainVoteCollectorVotes[sourceChain][proposalId];
