...
contract Factory4626 {
    ...
    /// @param rewardRecipient The address to receive rewards
    function deployMoonwellERC4626(address mToken, address rewardRecipient) external returns (address vault) {
        ...
        vault = address(new MoonwellERC4626(ERC20(asset), MErc20(mToken), rewardRecipient, moontroller));
        ...
        IERC20(asset).safeTransferFrom(msg.sender, address(this), initialMintAmount);

        IERC20(asset).safeApprove(vault, initialMintAmount);

        require(MoonwellERC4626(vault).deposit(initialMintAmount, address(0)) > 0, "deposit failed");
